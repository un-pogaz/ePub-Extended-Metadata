...
class Container(object):
    ...
    def get_spine_items(self):
        spine_items = self.opf.xpath('//opf:spine', namespaces={'opf':NS_OPF})[0]
    ...
    def get_toc_navpoint_content(self, item):
        ...
        xp = self.ncx.xpath('//ncx:navPoint', namespaces={'ncx':NS_NCX})
        if xp:
            for navpoint in xp:
                content = navpoint.xpath('ncx:content', namespaces={'ncx':NS_NCX})
                if len(content):
...
class WritableContainer(Container):
    ...
    def add_name_to_manifest(self, name, mt=None):
        ...
        if item is not None:
            return
        manifest = self.opf.xpath('//opf:manifest', namespaces={'opf':NS_OPF})[0]
        item = manifest.makeelement('{%s}item'%NS_OPF, nsmap={'opf':NS_OPF},
                href=self.name_to_href(name),
    ...
    def add_to_spine(self, id, index=-1):
        ...
        '''
        spine = self.opf.xpath('//opf:spine', namespaces={'opf':NS_OPF})[0]
        itemref = spine.makeelement('{%s}itemref'%NS_OPF, nsmap={'opf':NS_OPF},
                idref=id)
        if index >= 0:
    ...
    def add_to_metadata(self, name, id, index=-1):
        ...
        '''
        metadata = self.opf.xpath('//opf:metadata', namespaces={'opf':NS_OPF})[0]
    ...
    def delete_from_toc(self, item=None, item_name=None):
        ...
        def test_navpoint_for_removal(navpoint):
            src = navpoint.xpath('ncx:content/@src', namespaces={'ncx':NS_NCX})
            if len(src):
        ...
        for navpoint in self.ncx.xpath('//ncx:navPoint', namespaces={'ncx':NS_NCX}):
            if test_navpoint_for_removal(navpoint):
                ...
                for child in reversed(navpoint):
                    if child.tag == '{%s}navPoint'%NS_NCX:
...
class ExtendedContainer(WritableContainer):
    ...
    def flatten_toc(self):
        ...
        nested = self.ncx.xpath(r'descendant::ncx:navPoint/ncx:navPoint',
                                                   namespaces={'ncx':NS_NCX})
        if len(nested) == 0:
            self.log('\t  No nested navPoints')
            return False

        for navpoint in self.ncx.xpath('//ncx:navPoint', namespaces={'ncx':NS_NCX}):
            child_navpoints = navpoint.xpath('ncx:navPoint', namespaces={'ncx':NS_NCX})
